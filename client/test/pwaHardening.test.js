...
const serviceWorkerSource = fs.readFileSync(new URL('../public/carearound-sw.js', import.meta.url), 'utf8');
...
test('service worker registration wires the CareAround worker without touching app routes', async () => {
    ...
    assert.deepEqual(calls[0], ['register', '/carearound-sw.js', { scope: '/' }]);
...
test('offline and delivery files are present and service worker updates are not cached by Cloudflare Pages', () => {
    ...
    assert.match(headersSource, /\/carearound-sw\.js[\s\S]*Cache-Control: no-cache/);
