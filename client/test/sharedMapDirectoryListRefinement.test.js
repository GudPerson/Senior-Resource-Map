...
const compactResourcePreviewCardSource = readFileSync(
...
test('mobile map focus tray shows selected cards without changing list order', () => {
    ...
    assert.match(sharedMapDirectorySource, /mobileFocusCardVariant = 'default'/);
    assert.match(myMapV2ScaffoldSource, /mobileFocusCardVariant="complete-preview"/);
    assert.doesNotMatch(sharedMapPageSource, /mobileFocusCardVariant="complete-preview"/);
    assert.match(traySource, /cardVariant === 'complete-preview'/);
    assert.match(traySource, /<CompactResourcePreviewCard/);
    assert.match(traySource, /detailAction=\{/);
    assert.match(traySource, /<Link to=\{placeDetailPath\}/);
...
test('print V2 cards can opt into compact category-coloured resource badges', () => {
    ...
    assert.match(categoryPinShapeBadgeSource, /strokeWidth=\{getCategoryPinRingStrokeWidth\(normalizedRingWeight\)\}/);
    ...
    assert.match(sharedMapDirectorySource, /ringWeight=\{numberedPinStyle\?\.ringWeight\}/);
...
test('v2 card ordering can opt into category pills and integrated list-only cards', () => {
    ...
    assert.match(sharedMapDirectorySource, /buildCategoryBadgePalette\(color\)/);
    assert.match(sharedMapDirectorySource, /--directory-category-accent/);
    assert.match(sharedMapDirectorySource, /borderColor: 'var\(--directory-category-accent\)'/);
    assert.match(sharedMapDirectorySource, /color: palette\.textColor/);
