...
test('my map v2 mobile chrome stays compact in normal flow above the scrolling map', () => {
    ...
    assert.match(mobileControlsSource, /Drawer\.Overlay[\s\S]*?forceMount[\s\S]*?className="fixed inset-0 z-\[1200\]/);
    assert.match(mobileControlsSource, /Drawer\.Content[\s\S]*?forceMount[\s\S]*?className="fixed bottom-0 left-0 top-\[56px\] z-\[1210\]/);
...
test('my map v2 uses the restored normal map sizing without enabling full-map mode', () => {
    ...
    assert.match(myMapV2ScaffoldSource, /fixedTownSurfaceContainOnResize=\{useDesktopLayout && fixedTownSurfaceContainOnResize\}/);
    assert.match(myMapV2ScaffoldSource, /fixedTownSurfaceMaxDecodedBytes=\{fixedTownSurfaceMaxDecodedBytes\}/);
