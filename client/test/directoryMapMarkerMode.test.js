...
test('directory map can disable clustering so close pins behave as individual markers', () => {
    assert.match(directoryMapSource, /const shouldCluster = showPins && clusterMarkerMode !== 'none' && displayMarkerPins\.length > 1/);
});
...
test('directory map can hide pin artwork without losing its camera points or basemap', () => {
    ...
    assert.match(directoryMapSource, /const displayMarkerPins = useMemo/);
...
test('print badge collision offsets survive Leaflet marker redraws', () => {
    assert.doesNotMatch(directoryMapSource, /if \(!preserveSolvedOffsets\) return false/);
    assert.match(directoryMapSource, /if \(typeof MutationObserver !== 'undefined'\) \{/);
...
test('directory map can render interactive category bubble markers with visible lobe hit targets', () => {
    ...
    assert.match(directoryMapSource, /function isCategoryBubbleFixedToAnchor/);
    assert.match(directoryMapSource, /fixedPlaceKeys=\{collisionFixedPlaceKeys\}/);
    assert.match(directoryMapSource, /const fixed = isCategoryBubbleFixedToAnchor\(coreElement, fixedPlaceKeySet\)/);
