        ...
        <div className={`flex items-center gap-3 ${className}`}>
            ...
            <div className={`min-w-0 ${textClassName}`}>
                <div className="flex items-center gap-2">
                    <div
                        className={`truncate text-base font-extrabold sm:text-lg ${compact ? 'hidden sm:block' : ''}`}
                        style={{ color: 'var(--color-brand-strong)', fontFamily: 'var(--font-heading)' }}
                    >
                        CareAround SG
                    </div>
                    <span
                        className="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.18em] text-amber-700"
                ...
                {showTagline && (
                    <div
                        className={`mt-0.5 text-xs font-semibold sm:text-sm ${compact ? 'hidden sm:block' : ''}`}
