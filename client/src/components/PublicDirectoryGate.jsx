    ...
    return (
        <main className="min-h-[calc(100vh-80px)] px-4 py-14" style={{ background: 'var(--page-gradient)' }}>
            <div className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-7 shadow-xl sm:p-9">
                ...
                <h1 className="mt-5 text-3xl font-black text-slate-950">{onboardingEnabled ? 'CareAround SG is in an organisation-led pilot' : 'CareAround SG public access is currently restricted'}</h1>
                <p className="mt-3 text-base leading-7 text-slate-600">{!onboardingEnabled
                ...
                {onboardingEnabled && <p className="mt-6 text-sm text-slate-500">Is your organisation new to CareAround SG? <Link to="/organization/register" className="font-bold text-brand-700 hover:underline">Register the organisation</Link>.</p>}
            </div>
        </main>
    );
}
...
export default function PublicDirectoryGate({ children }) {
    ...
    if (!userCanUseDirectory(user, settings)) return <PublicAccessNotice settings={settings} />;
    return children;
}
