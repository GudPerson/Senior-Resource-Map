...
export function CategoryBadge({ category }) {
    ...
    if (isHard) {
        return (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-slate-900 text-sm font-bold border border-emerald-200">
                <Building2 size={14} className="text-emerald-600" /> Place
            </span>
        );
    }
    ...
    return (
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 text-slate-900 text-sm font-bold border border-slate-200">
            {category}
        </span>
    );
}
