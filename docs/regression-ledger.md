...
| Phone identity uniqueness | Phase 1 added a standalone Singapore account-phone normalizer for future WhatsApp/GudAuth login/linking preparation only; current login/session behavior and existing `users.phone` values remain unchanged; no uniqueness enforcement or identity table exists yet | `codex/phone-normalization-phase1` implementation | Run focused phone normalization tests; run session/auth regression tests before any future phone-login wiring; audit duplicate `users.phone` groups before adding a verified phone identity table or unique index | Singapore account identity phones normalize to E.164 (`+65...`) consistently; blank/invalid values fail safely; future WhatsApp/GudAuth account lookup must map a verified phone to zero or one CareAround account before enabling phone login; do not look up users from raw `users.phone` directly | 2026-05-06 |
