...
| Dashboard resources/admin | Recovered and locked on stabilization branch; punctuation-normalized resource search retested locally; dashboard resource logos render inside stable contained frames | User-approved stabilization branch behavior + 2026-05-20 logo-frame polish | Load `/dashboard/resources` and `/dashboard/admin`, test search, counts, export, and uploaded/enriched logos with wide, tall, and square artwork | Search stable, counts consistent, filtered workbook export works, admin search does not reload on every letter, names with parentheses remain searchable; resource, offering, and template logos fit fully within their frames without cropping or overflowing | 2026-05-20 |
