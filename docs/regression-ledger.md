...
| Auth transition handoff | Added explicit `/auth/transition` loading screen after successful login to avoid transient return-to-login flicker; transition route can surface announcements/tips before dashboard handoff | Commit `TBD` (this change) + user verification in local `/login` flows (email/Google/WhatsApp) | Sign in with a regular account via email, Google, or WhatsApp, then observe post-submit route behavior before and after membership-token returns; confirm `/auth/transition` shows loading UI and continues into `/dashboard` or intended `returnTo` destination | Users should always see a dedicated loading handoff screen after auth completion, not `/login`; transition remains within same route state if token checks are slow; users can continue manually after delay fallback without creating a login loop | 2026-05-16 |
