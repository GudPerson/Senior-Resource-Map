...
4. **Direct Gemini API with `gemini-3.1-flash-lite`** for bounded staff collateral extraction only, with staff review before anything is published.
...
| `GEMINI_API_MODEL` | `gemini-3.1-flash-lite` | Keeps collateral extraction on Google's supported cost-efficient multimodal model by default. |
...
Pricing checked on 2026-07-18:

- Google lists `gemini-3.1-flash-lite` as its cost-efficient multimodal model for high-volume and simple data-processing work, with standard text/image/video input at USD 0.25 per 1M tokens and text/thinking output at USD 1.50 per 1M tokens. Source: `https://ai.google.dev/gemini-api/docs/pricing`
- Google recommends `gemini-3.1-flash-lite` as the replacement for `gemini-2.5-flash-lite`; new-key production verification on 2026-07-18 confirmed that the older model returned HTTP 404 while the replacement completed the same collateral preview. Source: `https://ai.google.dev/gemini-api/docs/deprecations`
...
6. Set `AI_IMPORT_PROVIDER=gemini` and `GEMINI_API_MODEL=gemini-3.1-flash-lite` only after `GEMINI_API_KEY` exists in the Worker environment.
