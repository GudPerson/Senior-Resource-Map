...
function buildEnrichmentPrompt({ candidates, keywordQuery = '' }) {
    ...
    return [
        ...
        'The following places are existing CareAround drafts or Google Places candidates. Use Google Search grounding to cross-reference each one.',
        keywordLine,
        'For each place, find:',
        ...
        '  - address: the full street/block/unit address, not just "Singapore" plus postal code.',
        '  - postalCode: the official Singapore postal code.',
        '  - phone: the official contact telephone number, if clearly findable.',
        ...
        'When googlePlaceId or website is missing, search by the place name, postal code, and address.',
        'If the address is only "Singapore" plus a postal code, use the name and postal code to identify the official listing.',
...
function normalizeEnrichmentItem(raw) {
    return {
        ...
        hours: normalizeLongText(raw?.hours || raw?.openingHours || raw?.operatingHours),
